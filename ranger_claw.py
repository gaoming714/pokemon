...
def tape_archive():
    now_online = webDB.fetch_time()
    date_online = now_online.split()[0]
    source_path = os.path.join("data", "claw_data.json")
...
def update_nightly(date_online):
    nightly_path = os.path.join("data", "claw_nightly.json")
    json_path = os.path.join("data", "claw_data.json")
...
def archive_intraday(date_online):
    source = os.path.join("data", "claw_data.json")
    target = os.path.join("data", "claw." + date_online + ".json")
